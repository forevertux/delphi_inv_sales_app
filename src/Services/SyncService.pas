...
function TSyncService.JoinCSV(const List: TStringList): string;
var
  I: Integer;
begin
  Result := '';
...
begin
  ...
  try
    ...
    begin
      ...
      if SameText(Pair.JsonString.Value, PKField) then
        Continue;

      FieldNames.Add(Pair.JsonString.Value);
      Placeholders.Add(':' + Pair.JsonString.Value);
    end;
    ...
    if FieldNames.Count = 0 then
      ...
      Result := Format('INSERT INTO %s (%s) VALUES (%s)',
        [TableName, JoinCSV(FieldNames), JoinCSV(Placeholders)]);
  finally
    FieldNames.Free;
    Placeholders.Free;
  end;
...
begin
  ...
  try
    ...
    begin
      LocalRecord := GetRecordAsJSON(Query, TableName);
      try
        if (LocalRecord <> nil) and not HandleConflict(LocalRecord, RecordData) then
        begin
          Result := True;
          Exit;
        end;
