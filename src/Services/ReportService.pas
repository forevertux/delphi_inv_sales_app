...
begin
  ...
  DateGroupExpr := DMDatabase.SqlDateGroupExpr('s.SaleDate');
  ...
  try
    ...
    SQL :=
      ...
      '  COUNT(DISTINCT ' + DateGroupExpr + ') AS DaysWorked, ' +
      '  CASE ' +
      '    WHEN COUNT(DISTINCT ' + DateGroupExpr + ') > 0 THEN ' +
      '      SUM(s.TotalAmount) / COUNT(DISTINCT ' + DateGroupExpr + ') ' +
      '    ELSE 0 ' +
      '  END AS AverageDailySales ' +
      'FROM Users u ' +
      'LEFT JOIN Sales s ON u.UserID = s.EmployeeID ' +
      '  AND s.SaleDate >= :StartDate AND s.SaleDate < :EndDateExclusive ' +
