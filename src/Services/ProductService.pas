...
begin
  ...
  try
    ...
    Product.ProductID := DMDatabase.GetLastInsertID;
    Result := Product.ProductID > 0;
    if Result then
    begin
      if Assigned(GSyncService) then
        GSyncService.LogChange('Products', Product.ProductID, soInsert);
...
begin
  ...
  try
    ...
    begin
      if Assigned(GSyncService) then
        GSyncService.LogChange('Products', Product.ProductID, soUpdate);
...
begin
  ...
  try
    ...
    begin
      if Assigned(GSyncService) then
        GSyncService.LogChange('Products', ProductID, soDelete);
...
begin
  ...
  try
    try
      Query.SQL.Text :=
        'UPDATE Products SET Quantity = :Quantity, UpdatedAt = CURRENT_TIMESTAMP ' +
      ...
      if Result and Assigned(GSyncService) then
        GSyncService.LogChange('Products', ProductID, soUpdate);
    except
      on E: Exception do
...
begin
  ...
  if (ProductID <= 0) or (RequestedQty <= 0) then
    Exit;
  ...
  try
    try
      Query.SQL.Text :=
        'SELECT Quantity FROM Products WHERE ProductID = :ProductID AND IsActive = 1';
      Query.ParamByName('ProductID').AsInteger := ProductID;
      Query.Open;
