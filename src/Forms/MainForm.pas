...
begin
  ...
  if Assigned(AuthService) and AuthService.IsAuthenticated then
  begin
    lblUserInfo.Text := 'User: ' + AuthService.CurrentUser.FullName;
  end
  else
  begin
...
begin
  ...
  if Assigned(AuthService) and AuthService.IsAuthenticated then
  begin
    TabUsers.Visible := AuthService.CurrentUser.CanAccessUserManagement;
    TabReports.Visible := AuthService.CurrentUser.CanAccessReports;
  end
  else
  begin
...
begin
  try
    // Get today's sales
    TodaySales := SalesService.GetTodaysSales;
    TotalSalesToday := 0;
