...
begin
  // Check permissions
  if not AuthService.CurrentUser.CanAccessReports then
  begin
  ...
  try
    ...
    case cmbReportType.ItemIndex of
      0: // Sales Report
        DataSet := ReportService.GetSalesReport(StartDate, EndDate, BranchID);

      1: // Inventory Report
        DataSet := ReportService.GetInventoryReport(BranchID);

      2: // Top Selling Products
        DataSet := ReportService.GetTopSellingProducts(StartDate, EndDate, 20);

      3: // Sales by Category
        DataSet := ReportService.GetSalesByCategory(StartDate, EndDate);

      4: // Sales by Employee
        DataSet := ReportService.GetSalesByEmployee(StartDate, EndDate);

      5: // Low Stock Report
        DataSet := ReportService.GetLowStockReport;

      6: // Out of Stock Report
        DataSet := ReportService.GetOutOfStockReport;

      7: // Profit Analysis
        DataSet := ReportService.GetProfitAnalysis(StartDate, EndDate);
    else
...
begin
  ...
  if ReportService.ExportToCSV(FCurrentDataSet, FilePath) then
  begin
...
begin
  ...
  if ReportService.ExportToPDF(FCurrentDataSet, FilePath) then
  begin
    TDialogService.MessageDialog(
      Format('Report exported successfully to:%s%s', [sLineBreak, FilePath]),
      TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOK], TMsgDlgBtn.mbOK, 0,
